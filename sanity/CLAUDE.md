...
- **Documents and singletons** — always provide an `icon` using an emoji arrow function:
  ...
  icon: () => '📄',
  ```
- **Objects** — icons are optional but encouraged for clarity in the array item UI.
...
This is a monorepo. The Sanity Studio (`sanity/`) is the CMS backend for the Astro frontend (`astro/`). Schema changes here have a direct counterpart on the frontend.
